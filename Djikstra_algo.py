...
# Perform a graph traversal using Dijkstra's algorithm
traversal_path = list(nx.shortest_path(G, source=0, method='dijkstra'))
...
for u, v in zip(traversal_path[:-1], traversal_path[1:]):
    ...
    cv2.imshow("Face Traversal Animation (Dijkstra)", frm)
